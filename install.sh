...
flake::build() {
    ...
    [[ -z "$hostname" ]] && { log ERROR "Hostname required."; return 1; }
...
cmd_pre-install() {
    local hostname="${1:-$(config::get HOSTNAME)}"
    [[ -z "$hostname" ]] && { log ERROR "Hostname required (use --host or -H)."; return 1; }
...
cmd_install() {
    local hostname="${1:-$(config::get HOSTNAME)}"
    [[ -z "$hostname" ]] && { log ERROR "Hostname not specified."; return 1; }
