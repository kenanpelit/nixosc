...
echo "COMMAND: zen "-P" "Spotify" "--class" "Spotify" "--name" "Spotify" "--restore-session""
    ...
    bypass)
        if command -v mullvad >/dev/null 2>&1 && mullvad status 2>/dev/null | grep -q "Connected"; then
            if command -v mullvad-exclude >/dev/null 2>&1; then
                ...
                mullvad-exclude zen "-P" "Spotify" "--class" "Spotify" "--name" "Spotify" "--restore-session" &
            else
                echo "UYARI: mullvad-exclude bulunamadı, normal başlatılıyor"
                zen "-P" "Spotify" "--class" "Spotify" "--name" "Spotify" "--restore-session" &
            fi
        else
...
APP_PID=$!
mkdir -p "/tmp/sem"
echo "$APP_PID" > "/tmp/sem/Zen-Spotify.pid"
