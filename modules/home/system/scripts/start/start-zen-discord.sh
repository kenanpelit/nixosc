...
echo "COMMAND: zen "-P" "Discord" "--class" "Discord" "--name" "Discord" "--restore-session""
    ...
    bypass)
        if command -v mullvad >/dev/null 2>&1 && mullvad status 2>/dev/null | grep -q "Connected"; then
            if command -v mullvad-exclude >/dev/null 2>&1; then
                ...
                mullvad-exclude zen "-P" "Discord" "--class" "Discord" "--name" "Discord" "--restore-session" &
            else
                echo "UYARI: mullvad-exclude bulunamadı, normal başlatılıyor"
                zen "-P" "Discord" "--class" "Discord" "--name" "Discord" "--restore-session" &
            fi
        else
...
APP_PID=$!
mkdir -p "/tmp/sem"
echo "$APP_PID" > "/tmp/sem/Zen-Discord.pid"
