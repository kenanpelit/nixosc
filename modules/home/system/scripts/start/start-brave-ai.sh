...
echo "COMMAND: profile_brave Ai --class Ai --title Ai --restore-last-session"
    ...
    bypass)
        if command -v mullvad >/dev/null 2>&1 && mullvad status 2>/dev/null | grep -q "Connected"; then
            if command -v mullvad-exclude >/dev/null 2>&1; then
                ...
                mullvad-exclude profile_brave Ai --class Ai --title Ai --restore-last-session &
            else
                echo "WARNING: mullvad-exclude not found, starting normally"
                profile_brave Ai --class Ai --title Ai --restore-last-session &
            fi
        else
