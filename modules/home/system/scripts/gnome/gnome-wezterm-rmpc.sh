...
set -e
echo "Initializing wezterm-rmpc for GNOME/Wayland..."
...
if [[ "0" != "0" ]]; then
    ...
    if command -v gdbus >/dev/null 2>&1; then
        ...
        if [[ "$CURRENT_WORKSPACE" != "$TARGET_WORKSPACE" ]]; then
            echo "Switching to workspace 0 using gdbus (Wayland)..."
        ...
        if [[ "$CURRENT_WORKSPACE" != "$TARGET_WORKSPACE" ]]; then
            echo "Switching to workspace 0 using wmctrl (XWayland fallback)..."
...
APP_PID=$!
mkdir -p "/tmp/semsumo"
echo "$APP_PID" > "/tmp/semsumo/wezterm-rmpc.pid"
echo "Application started (PID: $APP_PID)"

# Wait for application to load and ensure correct workspace
sleep 3
if [[ "0" != "0" ]] && command -v gdbus >/dev/null 2>&1; then
