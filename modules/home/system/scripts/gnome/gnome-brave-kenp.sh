...
set -e
echo "Initializing brave-kenp for GNOME/Wayland..."
...
if [[ "1" != "0" ]]; then
    ...
    if command -v gdbus >/dev/null 2>&1; then
        ...
        if [[ "$CURRENT_WORKSPACE" != "$TARGET_WORKSPACE" ]]; then
            echo "Switching to workspace 1 using gdbus (Wayland)..."
        ...
        if [[ "$CURRENT_WORKSPACE" != "$TARGET_WORKSPACE" ]]; then
            echo "Switching to workspace 1 using wmctrl (XWayland fallback)..."
...
sleep 3
if [[ "1" != "0" ]] && command -v gdbus >/dev/null 2>&1; then
