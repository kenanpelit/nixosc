...
set -e
echo "Initializing mkenp for GNOME/Wayland..."
...
if [[ "2" != "0" ]]; then
    ...
    if command -v gdbus >/dev/null 2>&1; then
        ...
        if [[ "$CURRENT_WORKSPACE" != "$TARGET_WORKSPACE" ]]; then
            echo "Switching to workspace 2 using gdbus (Wayland)..."
        ...
        if [[ "$CURRENT_WORKSPACE" != "$TARGET_WORKSPACE" ]]; then
            echo "Switching to workspace 2 using wmctrl (XWayland fallback)..."
...
sleep 3
if [[ "2" != "0" ]] && command -v gdbus >/dev/null 2>&1; then
