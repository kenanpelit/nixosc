...
set -e
echo "Initializing brave-compecta for GNOME/Wayland..."
...
if [[ "4" != "0" ]]; then
    ...
    if command -v gdbus >/dev/null 2>&1; then
        ...
        if [[ "$CURRENT_WORKSPACE" != "$TARGET_WORKSPACE" ]]; then
            echo "Switching to workspace 4 using gdbus (Wayland)..."
        ...
        if [[ "$CURRENT_WORKSPACE" != "$TARGET_WORKSPACE" ]]; then
            echo "Switching to workspace 4 using wmctrl (XWayland fallback)..."
...
sleep 3
if [[ "4" != "0" ]] && command -v gdbus >/dev/null 2>&1; then
