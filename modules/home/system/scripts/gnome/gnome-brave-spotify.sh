...
set -e
echo "Initializing brave-spotify for GNOME/Wayland..."
...
if [[ "8" != "0" ]]; then
    ...
    if command -v gdbus >/dev/null 2>&1; then
        ...
        if [[ "$CURRENT_WORKSPACE" != "$TARGET_WORKSPACE" ]]; then
            echo "Switching to workspace 8 using gdbus (Wayland)..."
        ...
        if [[ "$CURRENT_WORKSPACE" != "$TARGET_WORKSPACE" ]]; then
            echo "Switching to workspace 8 using wmctrl (XWayland fallback)..."
...
sleep 3
if [[ "8" != "0" ]] && command -v gdbus >/dev/null 2>&1; then
