...
#######################################
#
# tmux-manager.sh - Birleşik Tmux Yönetim Aracı
#
# Version: 1.0.0
# Date: 2025-04-17
# Author: Kenan Pelit
...
create_layout() {
	...
	1)
		# Single panel layout
		tmux send-keys -t "$session_name" "tmux new-window -n 'kenp'" C-m
		tmux send-keys -t "$session_name" "tmux select-pane -t 1" C-m
		;;
	2)
		# Two panel layout
		tmux send-keys -t "$session_name" "tmux new-window -n 'kenp'" C-m
		tmux send-keys -t "$session_name" "tmux split-window -v -p 80" C-m
		tmux send-keys -t "$session_name" "tmux select-pane -t 2" C-m
		;;
	3)
		# Three panel L-shaped layout
		tmux send-keys -t "$session_name" "tmux new-window -n 'kenp'" C-m
		tmux send-keys -t "$session_name" "tmux split-window -h -p 80" C-m
		tmux send-keys -t "$session_name" "tmux select-pane -t 2" C-m
		tmux send-keys -t "$session_name" "tmux split-window -v -p 85" C-m
		tmux send-keys -t "$session_name" "tmux select-pane -t 3" C-m
		;;
	4)
		...
		tmux send-keys -t "$session_name" "tmux select-pane -t 1" C-m
		tmux send-keys -t "$session_name" "tmux split-window -v -p 80" C-m
		tmux send-keys -t "$session_name" "tmux select-pane -t 4" C-m
		;;
	5)
		# Five panel layout
		tmux send-keys -t "$session_name" "tmux new-window -n 'kenp'" C-m
		tmux send-keys -t "$session_name" "tmux split-window -h -p 70" C-m
		tmux send-keys -t "$session_name" "tmux split-window -h -p 50" C-m
		tmux send-keys -t "$session_name" "tmux select-pane -t 1" C-m
		tmux send-keys -t "$session_name" "tmux split-window -v -p 50" C-m
		tmux send-keys -t "$session_name" "tmux select-pane -t 2" C-m
		tmux send-keys -t "$session_name" "tmux split-window -v -p 50" C-m
		tmux send-keys -t "$session_name" "tmux select-pane -t 5" C-m
		;;
	*)
...
kenp_create_layout() {
	...
	# Doğrudan 3-panelli düzeni bu fonksiyon içinde oluşturalım
	tmux send-keys -t "$session_name" "tmux new-window -n 'kenp'" C-m
	tmux send-keys -t "$session_name" "tmux split-window -h -p 80" C-m
	tmux send-keys -t "$session_name" "tmux select-pane -t 2" C-m
	tmux send-keys -t "$session_name" "tmux split-window -v -p 85" C-m
...
# tm için yardım bilgisi
show_tm_help() {
	cat <<EOF
TmuxMaster - Gelişmiş Tmux Oturum ve Düzen Yöneticisi

Kullanım: $(basename "$0") tm [seçenekler] [oturum_adı]
...
show_help() {
	cat <<EOF
tmux-manager.sh v${VERSION} - Birleşik Tmux Yönetim Aracı
...
Modüller:
    ...
    tm         Orijinal tm komutunu çalıştır
...
Ortak Komutlar:
    ...
    $(basename "$0") kenp                        # KENP geliştirme oturumu başlat
...
main() {
	...
	"kenp" | "k")
		...
		;;
	"tm")
		# Doğrudan tm komutu gibi çalışan komut
		process_tm_commands "$@"
		;;
	"")
		show_help
