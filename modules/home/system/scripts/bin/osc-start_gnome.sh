...
readonly VERSION="7.0.0-gnome-fixed"
...
switch_workspace() {
	...
	if command -v gdbus >/dev/null 2>&1; then
		...
		if [[ "$current" != "$target_workspace" ]]; then
			...
			sleep 2 # Increased wait time for workspace switch
		else
...
force_window_to_workspace() {
	local target_workspace="$1"

	if [[ -z "$target_workspace" || "$target_workspace" == "0" || "$DRY_RUN" == "true" ]]; then
		return 0
	fi

	local target_index=$((target_workspace - 1))

	if command -v gdbus >/dev/null 2>&1; then
		log "INFO" "WORKSPACE" "Ensuring window is on workspace $target_workspace..."
		...
		gdbus call --session --dest org.gnome.Shell --object-path /org/gnome/Shell --method org.gnome.Shell.Eval "
			...
			if (window) {
				let workspace = global.workspace_manager.get_workspace_by_index($target_index);
...
generate_script() {
...

	{
		...
		echo "# Generated by Semsumo v$VERSION (GNOME Edition Fixed)"
		echo "set -e"
		...
		echo "fi"
		echo ""
		echo "# Switch to workspace FIRST (before starting app)"
		...
		echo "            sleep 2  # Increased wait time"
		echo "        else"
		...
		echo ""
		echo "# Wait for application to load and ensure correct workspace"
		echo "sleep 3"
		echo "if [[ \"$workspace\" != \"0\" ]] && command -v gdbus >/dev/null 2>&1; then"
		echo "    echo \"Ensuring window is on correct workspace...\""
		echo "    # Move focused window to target workspace if needed"
		echo "    TARGET_WORKSPACE=\$(($workspace - 1))"
		echo "    gdbus call --session --dest org.gnome.Shell --object-path /org/gnome/Shell --method org.gnome.Shell.Eval \""
		echo "        let window = global.display.get_focus_window();"
		echo "        if (window) {"
		echo "            let workspace = global.workspace_manager.get_workspace_by_index(\$TARGET_WORKSPACE);"
		echo "            window.change_workspace(workspace);"
		echo "        }"
		echo "    \" >/dev/null 2>&1"
		echo "fi"
		echo ""
...
launch_application() {
	...
	# Switch workspace before launching
	...
	# Force window to correct workspace
	force_window_to_workspace "$workspace"
...
launch_browsers() {
	...
	for profile in "${!browsers_ref[@]}"; do
		...
		sleep 2 # Stagger browser launches more
	done
}
	...
	for profile in "${daily_order[@]}"; do
		if [[ -v DAILY_PROFILES["$profile"] ]]; then
			...
			"TERMINALS")
				...
				sleep 2 # Extra delay for terminals
				;;
			"BRAVE_BROWSERS")
				config="${BRAVE_BROWSERS[$profile]}"
				launch_application "$profile" "$config" "brave"
				sleep 3 # Extra delay for browsers
				;;
			"APPS")
				config="${APPS[$profile]}"
...
list_profiles() {
	echo -e "${BOLD}${CYAN}Available Profiles (Browser: $BROWSER_TYPE, GNOME Edition Fixed):${NC}\n"
	...
	echo -e "\n${BOLD}${YELLOW}GNOME Fixes Applied:${NC}"
	echo -e "  • External monitor automatically set as primary"
	echo -e "  • Increased workspace switch delays (2s)"
	echo -e "  • Force window to correct workspace after launch"
	echo -e "  • Staggered application launches to prevent conflicts"
}
...
check_status() {
	echo -e "${BOLD}${CYAN}Application Status (GNOME Fixed):${NC}\n"
...
export_config() {
	log "INFO" "EXPORT" "Exporting current GNOME configuration (Fixed Edition)"

	echo -e "${BOLD}${CYAN}Current GNOME Configuration (Fixed):${NC}\n"
	...
	echo -e "\n${BOLD}GNOME Fixes:${NC}"
	echo -e "  • Enhanced workspace switching with increased delays"
	echo -e "  • Automatic external monitor primary setup"
	echo -e "  • Post-launch window workspace enforcement"
	echo -e "  • Staggered application launches"
	echo -e "  • Script prefix: gnome-"

	log "INFO" "EXPORT" "Fixed GNOME configuration displayed"
}
...
show_help() {
	echo -e "${BOLD}${GREEN}Semsumo v$VERSION - Enhanced Application Launcher & Generator (GNOME Edition Fixed)${NC}"
	echo
	...
	echo "    --help                Show this help"
	echo
	echo -e "${BOLD}GNOME Fixes (New):${NC}"
	echo "    • Automatic external monitor detection and primary setup"
	echo "    • Enhanced workspace switching with 2s delays"
	echo "    • Post-launch window workspace verification and correction"
	echo "    • Staggered launches to prevent workspace conflicts"
	...
	echo "    $0 launch --daily                   # Launch daily/essential profiles (fixed)"
	...
	echo "    Ensure 'Dynamic workspaces' is disabled in GNOME Settings"
	echo "    Set 'Fixed number of workspaces' to at least 10"
...
parse_args() {
	...
	help | --help | -h)
		...
		;;
	version | --version | -v)
		echo "Semsumo v$VERSION (GNOME Edition Fixed)"
		exit 0
		;;
	*)
...
main() {
	...
	mkdir -p "$LOG_DIR"
	log "INFO" "START" "Semsumo v$VERSION (GNOME Edition Fixed) started ($BROWSER_TYPE)" "true"
	...
	log "SUCCESS" "DONE" "All operations completed (GNOME Edition Fixed - $BROWSER_TYPE) - Time: ${total_time}s" "true"
}
