...
generate_script() {
...

	{
		...
		echo "# Generated by Semsumo v$VERSION (GNOME Edition)"
		echo "set -e"
...
list_profiles() {
	echo -e "${BOLD}${CYAN}Available Profiles (Browser: $BROWSER_TYPE, GNOME Edition):${NC}\n"
...
check_status() {
	echo -e "${BOLD}${CYAN}Application Status (GNOME):${NC}\n"
...
export_config() {
	log "INFO" "EXPORT" "Exporting current GNOME configuration (built-in profiles only)"

	# Just show the current configuration without creating external files
	echo -e "${BOLD}${CYAN}Current GNOME Configuration:${NC}\n"
	...
	echo -e "\n${BOLD}GNOME Features:${NC}"
	echo -e "  Workspace switching: gdbus (native Wayland)"
	echo -e "  Fullscreen: gdbus + wmctrl fallback"
	echo -e "  Script prefix: gnome-"

	log "INFO" "EXPORT" "GNOME configuration displayed (no external files created)"
}
...
show_help() {
	echo -e "${BOLD}${GREEN}Semsumo v$VERSION - Enhanced Application Launcher & Generator (GNOME Edition)${NC}"
	echo
	...
	echo "    --help                Show this help"
	echo
	echo -e "${BOLD}GNOME Features:${NC}"
	echo "    • Native Wayland workspace switching via gdbus"
	echo "    • XWayland fallback support with wmctrl"
	echo "    • Generated scripts prefix: gnome-"
	echo "    • GNOME Shell integration for fullscreen"
	echo
	echo -e "${BOLD}Examples:${NC}"
	echo "    $0 generate --all                   # Generate ALL GNOME scripts (all browsers)"
	echo "    $0 generate --daily                 # Generate daily/essential GNOME scripts only"
	echo "    $0 brave generate --browser-only    # Generate only Brave GNOME scripts"
	echo "    $0 zen generate zen-discord         # Generate specific Zen GNOME script"
	...
	echo "    Logs:    $LOG_FILE"
	echo
	echo -e "${BOLD}NixOS Integration:${NC}"
	echo "    Install dependencies: nix-env -iA nixpkgs.wmctrl nixpkgs.mullvad-vpn"
	echo "    Or add to configuration.nix: wmctrl, mullvad-vpn"
}
...
parse_args() {
	...
	help | --help | -h)
		...
		;;
	version | --version | -v)
		echo "Semsumo v$VERSION (GNOME Edition)"
		exit 0
		;;
	*)
...
main() {
	...
	mkdir -p "$LOG_DIR"
	log "INFO" "START" "Semsumo v$VERSION (GNOME Edition) started ($BROWSER_TYPE)" "true"
	...
	log "SUCCESS" "DONE" "All operations completed (GNOME Edition - $BROWSER_TYPE) - Time: ${total_time}s" "true"
}
