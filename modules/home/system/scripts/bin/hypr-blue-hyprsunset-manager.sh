...
# Description: HyprSunset Manager - Çalışan Versiyon
#
# License: MIT
#
#######################################
...
toggle_sunset() {
	if [ -f "$STATE_FILE" ]; then
		...
		if [ ! -f "$PID_FILE" ]; then
			...
			echo "Hypr Sunset yeniden başlatıldı"
			return
		fi
...
show_status() {
	...
	# Son sıcaklık bilgisi
	if [ -f "$LAST_TEMP_FILE" ]; then
		echo "  Son sıcaklık: $(cat "$LAST_TEMP_FILE")K"
	fi

	# Çalışan süreçler
	local process_count=$(pgrep -f hyprsunset 2>/dev/null | wc -l)
	echo "  Aktif süreçler: $process_count"
...
# CTRL+C ile düzgün çıkış - SADECE manuel çıkış için
cleanup() {
	if [ -f "$LAST_TEMP_FILE" ]; then
		hyprsunset -t $(cat "$LAST_TEMP_FILE") &
	fi
...
trap 'echo ""; echo "Program sonlandırılıyor..."; cleanup' SIGINT SIGTERM
