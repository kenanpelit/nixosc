...
# HyprSunset sıcaklık ayarları (zaman tabanlı)
TEMP_DAY=4050
TEMP_EVENING=4000
TEMP_NIGHT=3950
...
GAMMASTEP_TEMP_DAY=4500
GAMMASTEP_TEMP_NIGHT=3700
...
HYPRSUNSET PARAMETRELERI:
    ...
    --temp-evening VALUE    Akşam sıcaklığı (Kelvin, varsayılan: $TEMP_EVENING)
    --temp-night VALUE      Gece sıcaklığı (Kelvin, varsayılan: $TEMP_NIGHT)
    --temp-late VALUE       Geç gece sıcaklığı (Kelvin, varsayılan: $TEMP_LATE_NIGHT)
    --day-start VALUE       Gündüz başlangıç saati (0-23, varsayılan: $TIME_DAY_START)
    --evening-start VALUE   Akşam başlangıç saati (0-23, varsayılan: $TIME_EVENING_START)
    --night-start VALUE     Gece başlangıç saati (0-23, varsayılan: $TIME_NIGHT_START)
...
ÖRNEKLER:
    ...
    $(basename "$0") start --temp-night 3800 --gs-temp-night 3500
...
NOT:
    ...
    - Her iki araç da farklı renk katmanları oluşturarak istenen tonu elde eder
