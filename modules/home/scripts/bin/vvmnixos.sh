...
build_qemu_command() {
  ...
  # Enhanced USB and input device support
  cmd+=" -device usb-ehci,id=ehci"
  cmd+=" -device usb-host,bus=xhci.0"

  # Display and input configuration
  case "${CONFIG[display_mode]}" in
  gtk)
    cmd+=" -device virtio-vga-gl"
    cmd+=" -display gtk,gl=on,grab-on-hover=on"
    # Enhanced input device configuration
    cmd+=" -device usb-tablet,id=tablet"
    cmd+=" -device usb-mouse,id=mouse"
    cmd+=" -device virtio-keyboard-pci,id=keyboard"
    cmd+=" -device qemu-xhci,id=xhci2"
    cmd+=" -device usb-tablet,bus=xhci2.0"
    ;;
  spice)
    cmd+=" -device qxl-vga"
