...
echo "COMMAND: profile_brave Whats --separate --restore-last-session"
    ...
    bypass)
        if command -v mullvad >/dev/null 2>&1 && mullvad status 2>/dev/null | grep -q "Connected"; then
            if command -v mullvad-exclude >/dev/null 2>&1; then
                ...
                mullvad-exclude profile_brave Whats --separate --restore-last-session &
            else
                echo "WARNING: mullvad-exclude not found"
                profile_brave Whats --separate --restore-last-session &
            fi
        else
