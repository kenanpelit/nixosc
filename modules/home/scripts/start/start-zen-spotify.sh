...
if [[ "7" != "0" ]] && command -v hyprctl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    echo "Verifying window appearance on workspace 7..."
    ELAPSED=0
    CLASS_PATTERN="Spotify"
    
    while [[ $ELAPSED -lt $APP_TIMEOUT ]]; do
        if hyprctl clients -j 2>/dev/null | jq -e ".[] | select(.workspace.id == 7 and (.class | test(\"$CLASS_PATTERN\"; \"i\")))" >/dev/null 2>&1; then
            echo "Window verified on workspace 7 after ${ELAPSED}s"
            break
        fi
...
echo "zen-spotify initialization complete"
exit 0
