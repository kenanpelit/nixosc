...
readonly ARGS_STR="-P proxy --class Proxy --name Proxy --new-window --new-instance"
...
APP_PID=$!
mkdir -p "$STATE_DIR"
echo "$APP_PID" > "$STATE_DIR/firefox-proxy.pid"
echo "$COMMAND" > "$STATE_DIR/firefox-proxy.cmd"
