...
echo "Initializing firefox-proxy on $WM_TYPE..."
...
echo "COMMAND: firefox -P proxy --class Proxy --name Proxy --new-window --new-instance"
    ...
    bypass)
        if command -v mullvad >/dev/null 2>&1 && mullvad status 2>/dev/null | grep -q "Connected"; then
            if command -v mullvad-exclude >/dev/null 2>&1; then
                ...
                mullvad-exclude firefox -P proxy --class Proxy --name Proxy --new-window --new-instance &
            else
                echo "WARNING: mullvad-exclude not found"
                firefox -P proxy --class Proxy --name Proxy --new-window --new-instance &
            fi
        else
...
APP_PID=$!
mkdir -p "/tmp/semsumo"
echo "$APP_PID" > "/tmp/semsumo/firefox-proxy.pid"
...
echo "firefox-proxy initialization complete"
exit 0
