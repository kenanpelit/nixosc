...
readonly COMMAND="mpv"
readonly ARGS_STR="--player-operation-mode=pseudo-gui --input-ipc-server=/tmp/mpvsocket"
...
echo "Initializing mpv on $WM_TYPE..."
...
APP_PID=$!
mkdir -p "$STATE_DIR"
echo "$APP_PID" > "$STATE_DIR/mpv.pid"
echo "$COMMAND" > "$STATE_DIR/mpv.cmd"
