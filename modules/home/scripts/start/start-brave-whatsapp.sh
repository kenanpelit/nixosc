...
echo "COMMAND: profile_brave --whatsapp --class whatsapp --title whatsapp"
    ...
    bypass)
        if command -v mullvad >/dev/null 2>&1 && mullvad status 2>/dev/null | grep -q "Connected"; then
            if command -v mullvad-exclude >/dev/null 2>&1; then
                ...
                mullvad-exclude profile_brave --whatsapp --class whatsapp --title whatsapp &
            else
                echo "WARNING: mullvad-exclude not found, starting normally"
                profile_brave --whatsapp --class whatsapp --title whatsapp &
            fi
        else
...
APP_PID=$!
mkdir -p "/tmp/semsumo"
echo "$APP_PID" > "/tmp/semsumo/brave-whatsapp.pid"
echo "Application started (PID: $APP_PID)"

# Wait and verify application window appeared
if [[ "9" != "0" ]] && command -v hyprctl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    echo "Verifying window appearance on workspace 9..."
    ELAPSED=0
    CLASS_PATTERN="whatsapp"
    
    while [[ $ELAPSED -lt $APP_TIMEOUT ]]; do
        if hyprctl clients -j 2>/dev/null | jq -e ".[] | select(.workspace.id == 9 and (.class | test(\"$CLASS_PATTERN\"; \"i\")))" >/dev/null 2>&1; then
            echo "Window verified on workspace 9 after ${ELAPSED}s"
            break
        fi
...
echo "brave-whatsapp initialization complete"
exit 0
