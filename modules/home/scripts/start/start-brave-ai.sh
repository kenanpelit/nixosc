...
if [[ "3" != "0" ]] && command -v hyprctl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    echo "Verifying window appearance on workspace 3..."
    ELAPSED=0
    CLASS_PATTERN="brave"
    
    while [[ $ELAPSED -lt $APP_TIMEOUT ]]; do
        if hyprctl clients -j 2>/dev/null | jq -e ".[] | select(.workspace.id == 3 and (.class | test(\"$CLASS_PATTERN\"; \"i\")))" >/dev/null 2>&1; then
            echo "Window verified on workspace 3 after ${ELAPSED}s"
            break
        fi
...
echo "brave-ai initialization complete"
exit 0
