...
echo "Initializing brave-ai on $WM_TYPE..."
...
echo "COMMAND: profile_brave Ai --separate --restore-last-session"
    ...
    bypass)
        if command -v mullvad >/dev/null 2>&1 && mullvad status 2>/dev/null | grep -q "Connected"; then
            if command -v mullvad-exclude >/dev/null 2>&1; then
                ...
                mkdir -p "/tmp/semsumo"
                PID_FILE="/tmp/semsumo/brave-ai.pid"
                rm -f "$PID_FILE" 2>/dev/null || true
                mullvad-exclude profile_brave Ai --separate --restore-last-session --pid-file="$PID_FILE"
            else
                echo "WARNING: mullvad-exclude not found"
                mkdir -p "/tmp/semsumo"
                PID_FILE="/tmp/semsumo/brave-ai.pid"
                rm -f "$PID_FILE" 2>/dev/null || true
                profile_brave Ai --separate --restore-last-session --pid-file="$PID_FILE"
            fi
        else
...
APP_PID="$(tr -d '[:space:]' <"${PID_FILE:-/tmp/semsumo/brave-ai.pid}" 2>/dev/null || true)"
