...
echo "Initializing brave-youtube on $WM_TYPE..."
...
echo "COMMAND: profile_brave --youtube --separate --class brave-youtube.com__-Default"
    ...
    bypass)
        if command -v mullvad >/dev/null 2>&1 && mullvad status 2>/dev/null | grep -q "Connected"; then
            if command -v mullvad-exclude >/dev/null 2>&1; then
                ...
                mkdir -p "/tmp/semsumo"
                PID_FILE="/tmp/semsumo/brave-youtube.pid"
                rm -f "$PID_FILE" 2>/dev/null || true
                mullvad-exclude profile_brave --youtube --separate --class brave-youtube.com__-Default --pid-file="$PID_FILE"
            else
                echo "WARNING: mullvad-exclude not found"
                mkdir -p "/tmp/semsumo"
                PID_FILE="/tmp/semsumo/brave-youtube.pid"
                rm -f "$PID_FILE" 2>/dev/null || true
                profile_brave --youtube --separate --class brave-youtube.com__-Default --pid-file="$PID_FILE"
            fi
        else
...
APP_PID="$(tr -d '[:space:]' <"${PID_FILE:-/tmp/semsumo/brave-youtube.pid}" 2>/dev/null || true)"
...
if [[ "$WORKSPACE" != "0" && "$WM_TYPE" == "hyprland" ]] && command -v hyprctl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    ...
    ELAPSED=0
    CLASS_PATTERN="brave-youtube.com__-Default"
