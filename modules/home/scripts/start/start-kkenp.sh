...
APP_PID=$!
mkdir -p "/tmp/semsumo"
echo "$APP_PID" > "/tmp/semsumo/kkenp.pid"
echo "Application started (PID: $APP_PID)"

# Wait and verify application window appeared
if [[ "2" != "0" ]] && command -v hyprctl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    echo "Verifying window appearance on workspace 2..."
    ELAPSED=0
    CLASS_PATTERN="TmuxKenp"
    
    while [[ $ELAPSED -lt $APP_TIMEOUT ]]; do
        if hyprctl clients -j 2>/dev/null | jq -e ".[] | select(.workspace.id == 2 and (.class | test(\"$CLASS_PATTERN\"; \"i\")))" >/dev/null 2>&1; then
            echo "Window verified on workspace 2 after ${ELAPSED}s"
            break
        fi
...
echo "kkenp initialization complete"
exit 0
