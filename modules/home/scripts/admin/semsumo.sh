...
get_vpn_mode() {
  ...
  case "$cli_mode" in
  never | always | default)
    echo "$cli_mode"
    ;;
  "")
    if [[ -f "$CONFIG_FILE" ]]; then
      jq -r ".sessions.\"$session_name\".vpn_mode // \"default\"" "$CONFIG_FILE"
    else
      echo "default"
    fi
    ;;
  *)
    log_error "Geçersiz VPN modu: $cli_mode"
    return 1
    ;;
  esac
}
...
start_session() {
  ...
  always)
    if [[ "$vpn_status" != "true" ]]; then
      log_warn "VPN bağlantısı bulunamadı. Oturum 'never' modunda başlatılıyor: $session_name"
...
list_sessions() {
  ...
  jq -r '.sessions | to_entries[] | {
        ...
        vpn_mode: (.value.vpn_mode // "default"),
    ...
    while IFS= read -r line; do
      if [[ $line =~ :$ ]]; then
        ...
        mode=${BASH_REMATCH[1]}
        case "$mode" in
        always) printf "  VPN Modu: ${RED}%s${NC}\n" "$mode" ;;
        never) printf "  VPN Modu: ${GREEN}%s${NC}\n" "$mode" ;;
...
VPN Modları:
  never   : VPN dışında çalıştır
  always  : VPN zorunlu
  default : VPN durumundan bağımsız

Örnek Kullanımlar:
  # Oturum başlatma örnekleri
  sem start secure-browser         # Yapılandırma VPN modunu kullan
  sem start local-browser never    # VPN dışında çalıştır
  sem restart zen-browser always   # VPN içinde yeniden başlat
