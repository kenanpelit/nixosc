...
let
  ...
  bins = {
    ...
    niriOsc = "${config.home.profileDirectory}/bin/niri-osc";
    clipse = "clipse";
  ...
  niriConfigText = lib.concatStringsSep "\n" [
    ...
    # Runtime monitor/workspace profile generated by `niri-osc set init`.
    "include \"dms/monitor-auto.kdl\""
    settingsConfig.layout

    # Writable runtime overrides (used by niri-osc set, e.g. Zen Mode).
    "include \"dms/zen.kdl\""
...
in
{
  ...
  options.my.desktop.niri = {
    ...
    initArrangeWindows = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Run `niri-osc set go` during `niri-osc set init` on startup.";
    };
    ...
    enableWorkflow = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Enable daemon-free Niri workflow keybinds and helpers (niri-osc flow).";
    };
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
    {
      ...
      home.activation.niriZenConfig = dag.entryAfter [ "writeBoundary" ] ''
        ...
        # Migration: older niri-osc set zen wrote invalid inline KDL like `border { off }`.
