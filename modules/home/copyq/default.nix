...
let
  cfg = config.my.user.copyq;
in
{
  options.my.user.copyq = {
    enable = lib.mkEnableOption "CopyQ clipboard manager";
  };

  config = lib.mkIf cfg.enable {
    home.packages = [ pkgs.copyq pkgs.wl-clipboard ];
    ...
    xdg.configFile."copyq/copyq.conf".text = ''
      ...
      notes_css=
      notes_fg=#cdd6f4
      notes_font="Hack,13,-1,5,50,0,0,0,0,0"
      notification_bg=#313244
      notification_fg=#cdd6f4
      notification_font="Hack,13,-1,5,50,0,0,0,0,0"
      num_fg=#9399b2
      num_font="Hack,9,-1,5,25,0,0,0,0,0"
      num_margin=2
